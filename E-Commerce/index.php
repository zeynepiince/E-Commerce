...
if (!empty($_SESSION['user_id'])) {
    try {
        $stmtPreviouslyBought = $pdo->prepare("
            ...
            WHERE o.user_id = ?
            GROUP BY p.product_id, p.name, p.price, p.image_url, p.badges, c.category_name
  ...
  <nav class="categories-nav" id="categoriesNav">
    ...
    try {
        $stmt = $pdo->query("
            ...
            LEFT JOIN products p 
                ...
                AND p.sub_category <> ''
            ORDER BY c.category_name, p.sub_category
        ");
        ...
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
            ...
            if ($sub !== '' && !in_array($sub, $categories[$cat], true)) {
                $categories[$cat][] = $sub;
            }
        }
    ...
    ?>
      <div class="categories-nav-item">
        <a
          href="<?= htmlspecialchars(localized_path('products.php', ['category' => $cat]), ENT_QUOTES, 'UTF-8') ?>"
          class="categories-nav-label <?= $selectedCategory === $cat ? 'active' : '' ?>"
        >
  ...
  <div class="hero-slider">
    ...
    <div class="hero-slide hero-slide--active hero-slide--arrivals">
      ...
      <div class="hero hero--center">
        <div class="hero-copy">
          <h2 class="hero-title hero-title--arrivals">
            <?= htmlspecialchars(t("home.hero.new_arrivals", "New arrivals just dropped"), ENT_QUOTES, 'UTF-8') ?>
          </h2>
      ...
      <div class="hero hero--center">
        <div class="hero-copy">
          <h2 class="hero-title hero-title--editors">
            <?= htmlspecialchars(t("home.hero.editors", "Style picks from our editors"), ENT_QUOTES, 'UTF-8') ?>
          </h2>
          <p class="hero-subtitle">
            <?= htmlspecialchars(t("home.hero.editors_sub", "Minimal, timeless pieces that work from desk to dinner."), ENT_QUOTES, 'UTF-8') ?>
          </p>
          <a href="<?= htmlspecialchars(localized_path('products.php'), ENT_QUOTES, 'UTF-8') ?>" class="hero-cta hero-cta--outline">
            <?= htmlspecialchars(t("home.see_collection", "See Collection"), ENT_QUOTES, 'UTF-8') ?>
          </a>
        </div>
      </div>
    </div>
      ...
      <div class="hero hero--center">
        <div class="hero-copy">
          <h2 class="hero-title hero-title--sale">
            <?= htmlspecialchars(t("home.hero.weekend_sale", "Weekend sale on gadgets"), ENT_QUOTES, 'UTF-8') ?>
          </h2>
          <p class="hero-subtitle">
            <?= htmlspecialchars(t("home.hero.weekend_sale_sub", "Save on headphones, watches and accessories for 48 hours only."), ENT_QUOTES, 'UTF-8') ?>
          </p>
          <a href="<?= htmlspecialchars(localized_path('products.php', ['category' => 'electronics']), ENT_QUOTES, 'UTF-8') ?>" class="hero-cta">
            <?= htmlspecialchars(t("home.view_tech_deals", "View Tech Deals"), ENT_QUOTES, 'UTF-8') ?>
          </a>
        </div>
      </div>
    </div>
...
<section class="products flash-sale-section" id="flash-sale">
  ...
  <div class="featured-grid featured-grid--product-cards">
    <?php foreach ($dealProducts as $idx => $product): ?>
      <?php
        $badges = array_merge(
          get_product_badges($product, 'on-sale', $idx),
          [['key' => 'flash-deal', 'label' => 'Flash deal']]
        );
        include 'includes/product_card.php';
      ?>
    <?php endforeach; ?>
...
<section class="products wishlist-preview-section">
  ...
  <p style="margin-top:12px;font-size:13px;">
    <a href="<?= htmlspecialchars(localized_path('wishlist.php'), ENT_QUOTES, 'UTF-8') ?>" style="color:#ff6f00;text-decoration:none;">
...
<section class="products seasonal-section">
  ...
  <div class="seasonal-grid">
    <div class="seasonal-card">
      ...
      <button onclick="window.location.href='<?= htmlspecialchars(localized_path('products.php', ['seasonal' => 'spring']), ENT_QUOTES, 'UTF-8') ?>'">
        <?= htmlspecialchars(t("home.seasonal.spring_cta", "Shop the edit"), ENT_QUOTES, 'UTF-8') ?>
      </button>
    </div>
    ...
    <div class="seasonal-card">
      ...
      <button onclick="window.location.href='<?= htmlspecialchars(localized_path('products.php', ['seasonal' => 'tech']), ENT_QUOTES, 'UTF-8') ?>'">
        <?= htmlspecialchars(t("home.seasonal.tech_cta", "See tech picks"), ENT_QUOTES, 'UTF-8') ?>
      </button>
    </div>
    ...
    <div class="seasonal-card">
      ...
      <button onclick="window.location.href='<?= htmlspecialchars(localized_path('products.php', ['seasonal' => 'travel']), ENT_QUOTES, 'UTF-8') ?>'">
        <?= htmlspecialchars(t("home.seasonal.travel_cta", "Get ready"), ENT_QUOTES, 'UTF-8') ?>
      </button>
    </div>
  </div>
</section>
